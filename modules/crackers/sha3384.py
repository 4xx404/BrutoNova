...
class sha3384_hash:
	def __init__(self, sha3384_hash):
		self.sha3384 = sha3384_hash
	...
	def crack(self, option):
		...
		os.system('clear')
		print(banner)
		print(bc.BC + ' SHA3-384: ' + bc.GC + self.sha3384)
		print(bc.BC + ' Wordlist: ' + bc.GC + wordlist + '\n')
		match = ''
		for passwd in passwords.split('\n'):
			if(passwd != ''):
				pw = hashlib.sha3_384(bytes(passwd, 'utf-8')).hexdigest()
				if(self.sha3384 == pw):
