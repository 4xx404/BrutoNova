...
class sha3512_hash:
	def __init__(self, sha3512_hash):
		self.sha3512 = sha3512_hash
	...
	def crack(self, option):
		...
		os.system('clear')
		print(banner)
		print(bc.BC + ' SHA3-512: ' + bc.GC + self.sha3512)
		print(bc.BC + ' Wordlist: ' + bc.GC + wordlist + '\n')
		match = ''
		for passwd in passwords.split('\n'):
			if(passwd != ''):
				pw = hashlib.sha3_512(bytes(passwd, 'utf-8')).hexdigest()
				if(self.sha3512 == pw):
