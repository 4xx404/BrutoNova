...
class sha3224_hash:
	def __init__(self, sha3224_hash):
		self.sha3224 = sha3224_hash
	...
	def crack(self, option):
		...
		os.system('clear')
		print(banner)
		print(bc.BC + ' SHA3-224: ' + bc.GC + self.sha3224)
		print(bc.BC + ' Wordlist: ' + bc.GC + wordlist + '\n')
		match = ''
		for passwd in passwords.split('\n'):
			if(passwd != ''):
				pw = hashlib.sha3_224(bytes(passwd, 'utf-8')).hexdigest()
				if(self.sha3224 == pw):
