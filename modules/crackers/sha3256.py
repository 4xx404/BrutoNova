...
class sha3256_hash:
	def __init__(self, sha3256_hash):
		self.sha3256 = sha3256_hash
	...
	def crack(self, option):
		...
		os.system('clear')
		print(banner)
		print(bc.BC + ' SHA3-256: ' + bc.GC + self.sha3256)
		print(bc.BC + ' Wordlist: ' + bc.GC + wordlist + '\n')
		match = ''
		for passwd in passwords.split('\n'):
			if(passwd != ''):
				pw = hashlib.sha3_256(bytes(passwd, 'utf-8')).hexdigest()
				if(self.sha3256 == pw):
